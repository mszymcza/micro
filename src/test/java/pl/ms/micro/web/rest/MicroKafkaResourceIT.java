...
import pl.ms.micro.IntegrationTest;
import pl.ms.micro.config.EmbeddedKafka;
import pl.ms.micro.config.KafkaSseConsumer;
import pl.ms.micro.config.KafkaSseProducer;
...
class MicroKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/micro-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
